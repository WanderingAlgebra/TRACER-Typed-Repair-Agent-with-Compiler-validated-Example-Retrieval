...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `experiment_id` | string | 一次完整 pilot 的唯一标识；同次 A/B/C 运行保持一致 |
...
| `provider_error` | string 或 null | 有界且脱敏的 Provider 错误摘要；非空时不会调用编译器 |
| `usage` | object | Provider 报告的输入、输出和总 token |
| `estimated_cost_usd` | number 或 null | 根据可选价格配置估算的成本 |
...
| `candidate_policy` | object | 候选元编程阻断与最小子进程环境策略；正式运行必须为当前版本 |
| `timestamp_utc` | string | UTC 事件时间 |

原始源文件不会被覆盖。成功隔离文件保存在 `results/solutions/` 的条件子目录中；持续失败的最后候选保存在 `results/solutions/failures/`。

原始 JSONL 的 `source_file`、`compiler_command` 或诊断正文可能包含本机绝对路径，因此默认被 `.gitignore` 排除。正式交接使用 `python scripts/export_pilot.py --out <目录>`；该命令在人工复核门禁通过后解析每条 JSON，替换仓库、用户和临时目录前缀，扫描疑似凭据并生成校验和。不得把 `requests.sqlite3` 作为研究工件发布。
