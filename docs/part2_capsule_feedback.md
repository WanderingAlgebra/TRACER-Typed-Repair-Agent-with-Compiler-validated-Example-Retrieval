...
4. **配对门禁（已实现并完成正式运行）**：`scripts/validate_part2_pairing.py` 严格检查两组首轮候选、模型、endpoint、Responses wire API、响应存储、推理强度、预算和题集一致，并要求 Capsule 的 Memory/额外编译/额外 LLM 调用为零。FATE-M 25 题正式结果 25/25 通过，交接包位于 `results/handoff/part12-live-20260828/`。
